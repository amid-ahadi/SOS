...
class cases_model extends CI_Model 
{
    ...
    // New function to get the next case number
	function get_next_case_no()
	{
		$this->db->select_max('case_no');
		$query = $this->db->get('cases');
		$row = $query->row();
		
		if ($row && $row->case_no !== null) {
			$max_case_no = intval($row->case_no);
			if ($max_case_no < 1000) {
				return 1000;
			}
			return $max_case_no + 1;
		} else {
			// No cases exist yet, or case_no is NULL for all, start from 1000
			return 1000;
		}
	}
